...
path_image = "/home/nai/sub0405" # location of original images
resize_folder = "/home/nai/sub0405_144/"
char_train_name = "/home/nai/CNN_SJ/data_file/L691Ach_train.csv"
char_test_name  = "/home/nai/CNN_SJ/data_file/L691Ach_test.csv"
img_rows = 144
img_cols = 144

out_data = "sub0405_mytry.bin" # data file
nb_classes = 3
norm_tp = "SDSP1"
rate_train = 0.6
...
out_feat = "sub0405_feature_mytry.bin"
...
for image_type in folder_image:
    ...
    for image in os.listdir(path_image + "/" + image_type):
        ...
        if image_name in img_train[:,1]:
            ...
            X_train[i_train, :, :, :] = img
            if image_type!='PXXU1': y_train.append(  int( img_train[ list(img_train[:,1]).index(image_name),0 ]=='"NG"' )  )
            ...
            X_valid[i_valid, :, :, :] = img
            if image_type!='PXXU1': y_valid.append(  int( img_valid[ list(img_valid[:,1]).index(image_name),0 ]=='"NG"' )  )
            else: y_valid.append( 2 )
            i_valid += 1
            ...
            X_test[i_test, :, :, :] = img
            if image_type!='PXXU1': y_test.append(  int( img_test[ list(img_test[:,1]).index(image_name),0 ]=='"NG"' )  )
...
print 'Y_train 0, 1, 2:', sum(Y_train[:,0]), sum(Y_train[:,1]), sum(Y_train[:,2])
print 'X_valid Y_valid', X_valid.shape, Y_valid.shape
print 'Y_valid 0, 1, 2:', sum(Y_valid[:,0]), sum(Y_valid[:,1]), sum(Y_valid[:,2])
print 'X_test Y_test', X_test.shape, Y_test.shape
print 'Y_test 0, 1, 2:', sum(Y_test[:,0]), sum(Y_test[:,1]), sum(Y_test[:,2])
