...
path_image = "/home/nai/L691A_original" # location of original images
resize_folder = "/home/nai/L691A_original_" + str(img_size) + "/"
...
#### resize and save
folder_image = os.listdir(path_image)
X_data  = np.zeros( (n, img_rows, img_cols, 3) )
y_data  = []
...
for image_type in folder_image:
    ...
    for image in os.listdir(path_image + "/" + image_type):
        ...
        X_data[i_data, :, :, :] = img
        y_data.append(  int( image_type==norm_tp )  )
        #if i_train % 10000 ==0: print 'saving X_train', i_train
        i_data += 1
            
print 'X_data y_data', X_data.shape, len(y_data)
...
np.random.seed(1234)
shuffle_data = np.random.choice(X_data.shape[0], X_data.shape[0], replace=False)
X_data = X_data[ shuffle_data,:,:,: ]
Y_data = Y_data[ shuffle_data,:     ]

train_cut = round( X_data.shape[0]*rate_train )
valid_cut = round( X_data.shape[0]*(rate_train+rate_valid) )
X_train = X_data[ :train_cut,:,:,: ]
Y_train = Y_data[ :train_cut,: ]
X_valid = X_data[ train_cut:valid_cut,:,:,: ]
Y_valid = Y_data[ train_cut:valid_cut,: ]
X_test  = X_data[ valid_cut:,:,:,: ]
Y_test  = Y_data[ valid_cut:,: ]
