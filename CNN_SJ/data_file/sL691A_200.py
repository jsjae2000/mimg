...
path_image = "/home/nai/sL691A"
resize_folder = "/home/nai/sL691A_200/"
...
out_data = "sL691A_200.bin" # data file
nb_classes = 2
norm_tp = "SDSP1"
rate_train = 0.6
...
for image_type in folder_image:
    ...
    for image in os.listdir(path_image + "/" + image_type):
        ...
        if image_name in img_train[:,1]:
            ...
            if i_train % 1000 ==0: print 'saving X_train', i_train
            i_train += 1
        elif image_name in img_valid[:,1]:
            X_valid[i_valid, :, :, :] = img
            y_valid.append(  int( img_valid[ list(img_valid[:,1]).index(image_name),0 ]=='"NG"' )  )
            if i_valid % 1000 ==0: print 'saving X_valid', i_valid
            i_valid += 1
        elif image_name in img_test[:,1]:
            X_test[i_test, :, :, :] = img
            y_test.append(  int( img_test[ list(img_test[:,1]).index(image_name),0 ]=='"NG"' )  )
            if i_test % 1000 ==0:  print 'saving X_test' , i_test
            i_test +=1
            
print 'X_train X_valid X_test', X_train.shape, X_valid.shape, X_test.shape
...
##    for image in os.listdir(path_image + "/" + image_type):
##        image_name = '"'+image[0:-4].upper()+'"'
##        if image_name in img_train[:,1]: index_train.append(  list(img_train[:,1]).index(image_name)  )
##        elif image_name in img_valid[:,1]: index_valid.append(  list(img_valid[:,1]).index(image_name)  )
##        elif image_name in img_test[:,1]: index_test.append(  list(img_test[:,1]).index(image_name)  )
##
